...
long_put_contract = build(MyAmericanPutContractModel, (
    K = 75, premium = 10.7, sense = 1, copy = 1
));

short_put_contract = build(MyAmericanPutContractModel, (
    K = 65, premium = 3.30, sense = -1, copy = 1
));

# build the payoff and probability functions -
number_of_points = 1000;
S = range(40, stop=85, length = number_of_points) |> collect;
...
plot!(payoff_array[:,1], payoff_array[:,2], c=:red, lw=1, label="Long Put (K₂ = 75)", linestyle=:dash)
plot!(payoff_array[:,1], payoff_array[:,3], c=:blue, lw=1, label="Short Put (K₁ = 65)", linestyle=:dash)
...
plot!(profit_array[:,1], profit_array[:,2], c=:red, lw=1, label="Long Put (K₂ = 75)", subplot=2, linestyle=:dash)
plot!(profit_array[:,1], profit_array[:,3], c=:blue, lw=1, label="Short Put (K₁ = 65)", subplot=2, linestyle=:dash)
...
# dump -
savefig(joinpath(_PATH_TO_FIGS, "Fig-put-vertical-spread-debit-bear.svg"))
